...
class ProfileService {
  ...
  Future<ImageDTO> uploadProfileImage(File imageFile) async {
    ...
    if (response.statusCode == 200) {
      ...
      final apiResponse = ApiResponse<ImageDTO>.fromJson(
        json, (data) => ImageDTO.fromJson(data),
      );

      return apiResponse.data;
    } else {
      throw Exception('Error al subir la imagen: código ${response.statusCode}, ${response.statusMessage}');
    }
  }
  ...
  Future<ImageDTO> updateProfileImage(File imageFile) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Error al  actualizar la imagen: código ${response.statusCode}, ${response.statusMessage}');
    }
  }
  ...
  Future<void> deleteProfileImage() async{
    ...
    if(response.statusCode != 204){
      throw Exception('Error al eliminar la imagen: código ${response.statusCode}, ${response.statusMessage}');
    }
  }
}